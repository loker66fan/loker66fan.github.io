...
- 基于 Vue 驱动的单页首页
- 搜索弹层、搜索引擎切换、内置图标方案与自定义搜索引擎
- 快速搜索预设词
- 常用网站卡片导航
- 实时时间与一言展示
- 基于 `wttr.in` 的天气弹窗，包含中文预报整理、超时控制和本地缓存
- 支持网易云歌单 ID 配置的音乐播放器
- 壁纸切换、本地持久化与随机图源刷新
- 面向移动端的布局和交互优化
...
建议通过 `http://` 或 `https://` 打开页面，不要直接使用 `file://` 打开 `index.html`。部分浏览器会在本地文件模式下拦截天气请求。
